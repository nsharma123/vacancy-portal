...
if(!class_exists('THJMF_Post_Fields')):
	...
	class THJMF_Post_Fields{
		...
		private function get_job_feature_fields_old(){
			$arr = [];
			$settings = THJMF_Utils::get_default_settings();
			$arr['job_feature'] = isset( $settings['job_detail']['job_feature']['job_def_feature'] ) ? $settings['job_detail']['job_feature']['job_def_feature'] : "";
			$arr['job_feature_details'] = isset( $settings['job_detail']['job_feature']['job_def_feature_details'] ) ? $settings['job_detail']['job_feature']['job_def_feature_details'] : "";
			$arr['job_expiry'] = isset( $settings['job_detail']['job_expiry'] ) ? $settings['job_detail']['job_expiry'] : "";
			return $arr;
		}
		
		private function add_updated_global_features_old($arr, $change){
			if( isset( $arr['job_feature'] ) && isset( $arr['job_feature_details'] ) ){
				foreach ($change as $key => $value) {
					if( $key != '' && $value != ''){
						$arr['job_feature'][$key] = $value;
						$arr['job_feature_details'][$key] = '';
					}
				}
			}
			return $arr;
		...
		public function load_post_meta_data_old(){
			global $post;
			$field_arr = [];
			$settings = false;
			$feature_atts = array('job_feature', 'job_feature_details');
			$this->post_data = THJMF_Utils::get_post_meta_datas($post->ID);
			if($this->post_data && isset($this->post_data[0])){
				$settings = $this->post_data[0];
				$field_arr['job_feature'] = isset( $settings['features']['job_feature'] ) ? $settings['features']['job_feature'] : "";
				$field_arr['job_feature_details'] = isset( $settings['features']['job_feature_details'] ) ? $settings['features']['job_feature_details'] : "";
				$feature_keys = THJMF_Utils::get_job_feature_keys();
				if( is_array( $feature_keys ) && is_array($field_arr['job_feature']) ){
					$diff = array_diff($feature_keys, $field_arr['job_feature']);
					if( is_array( $diff ) && !empty( $diff) ){
						$field_arr = $this->add_updated_global_features( $field_arr, $diff);
					}
				}else{
					$field_arr = $this->get_job_feature_fields();
				}

				$field_arr['job_expiry'] = isset( $this->post_data['job_expiry'] ) && !empty( $this->post_data['job_expiry'] ) ? THJMF_Utils::convert_date_wp( $this->post_data['job_expiry'], true ) : "";
			}else{
				$settings = THJMF_Utils::get_default_settings();
				$field_arr['job_feature'] = isset( $settings['job_detail']['job_feature']['job_def_feature'] ) ? $settings['job_detail']['job_feature']['job_def_feature'] : "";
				$field_arr['job_feature_details'] = isset( $settings['job_detail']['job_feature']['job_def_feature_details'] ) ? $settings['job_detail']['job_feature']['job_def_feature_details'] : "";
				$field_arr['job_expiry'] = isset( $settings['job_detail']['job_expiry'] ) ? $settings['job_detail']['job_expiry'] : "";
			}
		...
		public function get_field_form_props(){
			$field_type = array('' => 'Select a type', 'text'=>'Text', 'select'=>'Select', 'radio' => 'Radio');
			return array(
		...
		private function get_settings_url(){
			$url = '<a href="'.esc_url( THJMF_Utils::get_admin_url('general') ).'">'.__('Create','job-manager-career').'</a>';
			$message = ' fields from plugin settings submenu to add job features';
